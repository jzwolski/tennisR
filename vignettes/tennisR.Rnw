...
Each tournament website is then broken down in text form to determine the beginning and end dates for the tournament, and the size of the draw \citet{htmlToText}.
...
Within the \texttt{for} loop for each tournament, another \texttt{for} loop is created that steps through each individual match within a tournament.  This \texttt{for} loop uses the results stored in files.2 for each tournament.  Using the \pkg{RCurl} package, the source code for each individual match can be found.
...
Given the initial dataset of match statistics, graphs can be produced to look at trends among players, tournaments, or any variable in the dataset.  Figure \ref{fig:murray} shows Andy Murray's first serve percentage from June 1, 2015 to the end of his U.S. Open run in early September.
...
Other packages that were used in assembling the match statistics dataset are \pkg{lubridate} \citep{lubridate}, \pkg{plyr} \citep{plyr}, \pkg{RCurl} \citep{RCurl}, \pkg{scrapeR} \citep{scrapeR}, and \pkg{stringr} \citep{stringr}.
...
Next, the names for the players for each match were used for the first two inputs in the \texttt{Predict} function.  The function allows for a vector to be used as an input, and an example of players who are represented as Player 1 in their respective matches is shown below.

<<eval=FALSE>>=
player1 <- c("Paolo Lorenzi","Malek Jaziri","Lleyton Hewitt","Ricardas Berankis")
@

A vector for the players who are represented as Player 2 in their respective matches must also be created, and used as the second input in the \texttt{Predict} function.
...
Next, the predictions are now compared to the actual results.  Again a vector of winning players must be created, and an example is shown below:
...
To further test the accuracy of the predictions, the players were divided into groups based on their elo rating.  The first test split the players into two groups, one group for the top twenty players based on the elo rating, and the other group for all the other players.  Thirty three matches occurred during the 2015 Miami Open that contained one player from each group.  The accuracy of the predictions for these thirty three matches was about 76\%.  A second test again split the players into two groups, one group for the top ten players based on the elo ratings, and the other group for all the other players.  Twenty three matches occurred during the 2015 Miami Open that contained one player from each group.  The accuracy of the predictions for these twenty three matches was about 87\%.  The \texttt{Predict} function does well in correctly predicting matches where one of the players is a top rated player.  It is more difficult to predict matches between two players of similar rating, since these players tend to have very similar ability.
...
The website FiveThirtyEight has published articles about tennis analytics, and how data is being used now more than ever in tennis.  One example is an article about the length of tennis matches, and the trend in the number of seconds between points over the last few years \citet{fivethirtyeight}.

IBM is very committed to tennis analytics, and their IBM Slamtracker is very useful in tracking match statistics during the course of the match \citet{ibm}.

\section*{Acknowledgements}

Plots are made with the R package \texttt{ggplot2} \citep{hadley:2009}.
